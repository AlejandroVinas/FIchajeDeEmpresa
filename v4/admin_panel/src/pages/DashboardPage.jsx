...
function AdminDashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const empleadosDentro = getEmpleadosDentro(fichajes).length;
  ...
  return (
    <div className="grid gap-6">
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        ...
        <StatCard label="Trabajando ahora" value={loading ? '…' : stats.empleadosDentro} />
      </section>
          ...
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {fichajes.slice(0, 8).map((item) => (
                <tr key={item.id} className="border-b last:border-b-0">
                  ...
                  <td className="py-3 pr-4 text-gray-600">
                    {formatHours(item.horas_jornada)} h · {item.hora_entrada || '—'}-{item.hora_salida || '—'}
                  </td>
              ...
              {!fichajes.length && !loading ? (
                <tr>
                  <td colSpan="5" className="py-6 text-center text-gray-500">Todavía no hay fichajes.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
...
function getEmpleadosDentro(fichajes) {
  const latestByEmployee = new Map();

  for (const fichaje of fichajes) {
    if (!latestByEmployee.has(fichaje.empleado_id)) {
      latestByEmployee.set(fichaje.empleado_id, fichaje);
    }
  }

  return [...latestByEmployee.values()].filter((item) => item.tipo === 'entrada');
}
