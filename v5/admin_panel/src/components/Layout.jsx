      ...
      <header className="border-b bg-white">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
          <div>
            ...
            <h1 className="text-2xl font-bold">{user?.role === 'admin' ? 'Panel de administracion' : 'Panel del empleado'}</h1>
          </div>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:px-8">
        <aside className="card h-fit p-3">
          <nav className="flex flex-col gap-2">
            ...
            {user?.role === 'admin' ? <NavLink to="/configuracion" className={linkClass}>Configuracion</NavLink> : null}
